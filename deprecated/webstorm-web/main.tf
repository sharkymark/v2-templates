...
resource "coder_agent" "dev" {
  ...
  startup_script = <<EOF
    ...
    /home/coder/.local/bin/projector config add webstorm1 /opt/webstorm --force --use-separate-config --port 9001 --hostname localhost
    /home/coder/.local/bin/projector run webstorm1 &
...
resource "kubernetes_pod" "main" {
  ...
  spec {
    ...
    container {
      name    = "dev"
      image   = "docker.io/marktmilligan/webstorm-chmod:latest"
