...
variable "image" {
  ...
  EOF
  default = "marktmilligan/idea-comm-multi-vscode:latest"
  validation {
    condition = contains([
      "marktmilligan/idea-comm-multi-vscode:latest"
    ], var.image)
...
~/.local/bin/projector run IntelliJ1 &
~/.local/bin/projector run IntelliJ2 &
...
resource "coder_app" "idea-1" {
  agent_id      = coder_agent.coder.id
  name          = "IDEA Community 1"
  icon          = "/icon/intellij.svg"
  url           = "http://localhost:8997/"
  relative_path = true
}

resource "coder_app" "idea-2" {
  agent_id      = coder_agent.coder.id
  name          = "IDEA Community 2"
  icon          = "/icon/intellij.svg"
