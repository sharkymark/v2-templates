...
resource "coder_agent" "dev" {
  ...
  startup_script = <<EOF
    ...
    # Assumes you have JetBrains IDE in /opt
    ...
    /home/coder/.local/bin/projector config add clion1 /opt/clion --force --use-separate-config --port 9001 --hostname localhost
    /home/coder/.local/bin/projector run clion1 &

    # clone 2 Java repos
    mkdir -p ~/.ssh
    ssh-keyscan -t ed25519 github.com >> ~/.ssh/known_hosts
    git clone --progress git@github.com:sharkymark/coder-react.git &
...
resource "kubernetes_pod" "main" {
  ...
  spec {
    ...
    container {
      name    = "dev"
      image   = "docker.io/marktmilligan/clion-chmod:latest"
