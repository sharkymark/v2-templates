...
# clone a repo
module "git-clone-bookmarks" {
    source   = "https://registry.coder.com/modules/git-clone"
    agent_id = coder_agent.dev.id
    url      = "https://github.com/sharkymark/db"
}
...
module "git-clone" {
    source   = "https://registry.coder.com/modules/git-clone"
    agent_id = coder_agent.dev.id
    url      = "https://github.com/sharkymark/soysoys"
}
...
resource "coder_script" "shutdown" {
  ...
  #!/bin/sh 
    ...
    kill $(lsof -i :3001 -t) >/tmp/pid.log 2>&1 &
  EOF
}

# bookmarks
resource "coder_app" "bookmarks" {
  agent_id = coder_agent.dev.id
  slug          = "bookmarks"  
