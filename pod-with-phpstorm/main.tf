...
resource "coder_agent" "dev" {
  ...
  startup_script = <<EOF
    ...
    # Configure and run JetBrains IDE with projector

    # Assumes you have php (/opt/ps)
    ...
    /home/coder/.local/bin/projector config add ps1 /opt/ps --force --use-separate-config --port 9001 --hostname localhost
    /home/coder/.local/bin/projector run ps1 &

    /home/coder/.local/bin/projector config add ps2 /opt/ps --force --use-separate-config --port 9002  --hostname localhost
    /home/coder/.local/bin/projector run ps2 &
