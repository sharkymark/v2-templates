...
resource "coder_agent" "project-1" {
  os   = "linux"
  arch = "amd64"
  dir = "/home/coder"
  startup_script = <<EOT
...
if [ -d "$PROJECTOR_CONFIG_PATH" ]; then
    ...
    grep -iv "HANDSHAKE_TOKEN" /home/coder/.projector/configs/intellij/run.sh > temp && mv temp /home/coder/.projector/configs/intellij/run.sh 2>&1 | tee -a projector.log
    chmod +x /home/coder/.projector/configs/intellij/run.sh 2>&1 | tee -a projector.log
...
resource "coder_agent" "project-2" {
  os   = "linux"
  arch = "amd64"
  dir = "/home/coder"
  startup_script = <<EOT
...
if [ -d "$PROJECTOR_CONFIG_PATH" ]; then
    ...
    echo 'autoinstalling IDE and creating projector config folder'
    /home/coder/.local/bin/projector ide autoinstall --config-name "intellij" --ide-name "${var.jetbrains-ide}" --hostname=localhost --port 8998 --use-separate-config --password coder 2>&1 | tee -a projector.log   
...
resource "coder_app" "intellij-1" {
  agent_id      = coder_agent.project-1.id
...
resource "coder_app" "intellij-2" {
  agent_id      = coder_agent.project-2.id
...
resource "kubernetes_pod" "main" {
  ...
  depends_on = [
    kubernetes_persistent_volume_claim.home-directory-1,
    kubernetes_persistent_volume_claim.home-directory-2
  ]    
  metadata {
  ...
  spec {
    ...
    container {
      name    = "java-intellij-1"
      image   = "docker.io/${var.image}"
      command = ["sh", "-c", coder_agent.project-1.init_script]
      security_context {
        run_as_user = "1000"
      }
      env {
        name  = "CODER_AGENT_TOKEN"
        value = coder_agent.project-1.token
      }
      resources {
        requests = {
          cpu    = "500m"
      ...
      volume_mount {
        ...
        name       = "home-directory-1"
      }      
    }
    container {
      name    = "java-intellij-2"
      image   = "docker.io/${var.image}"
      command = ["sh", "-c", coder_agent.project-2.init_script]
      security_context {
        run_as_user = "1000"
      }
      env {
        name  = "CODER_AGENT_TOKEN"
        value = coder_agent.project-2.token
      }
      resources {
        requests = {
          cpu    = "500m"
      ...
      volume_mount {
        mount_path = "/home/coder"
        name       = "home-directory-2"
      }      
    }    
    volume {
      name = "home-directory-1"
      persistent_volume_claim {
        claim_name = kubernetes_persistent_volume_claim.home-directory-1.metadata.0.name
      }
    }  
    volume {
      name = "home-directory-2"
      persistent_volume_claim {
        claim_name = kubernetes_persistent_volume_claim.home-directory-2.metadata.0.name
      }
    }             
  }
}

resource "kubernetes_persistent_volume_claim" "home-directory-1" {
  metadata {
    name      = "home-coder-${data.coder_workspace.me.owner}-${lower(data.coder_workspace.me.name)}-1"
...
resource "kubernetes_persistent_volume_claim" "home-directory-2" {
  metadata {
    name      = "home-coder-${data.coder_workspace.me.owner}-${lower(data.coder_workspace.me.name)}-2"
    namespace = var.workspaces_namespace
  }
  spec {
    access_modes = ["ReadWriteOnce"]
